...
export async function replyFromFinBuddy({ message, history, apiKey, model = "gemini-2.0-flash" }) {
  if (!apiKey) { const error = new Error("not-configured"); error.code = "not-configured"; throw error; }
  const question = typeof message === "string" ? message.trim().slice(0, 3000) : "";
  if (!question) { const error = new Error("empty-message"); error.code = "empty-message"; throw error; }
  ...
  } catch (error) { const scopedError = new Error("scope-unavailable"); scopedError.code = "scope-unavailable"; throw scopedError; }
  const scope = parseScope(scopeResponse.text);
  if (!scope.allowed) return { answer: FINBUDDY_REFUSAL, allowed: false, category: scope.category };
  try {
  ...
  } catch (error) { const answerError = new Error("answer-unavailable"); answerError.code = "answer-unavailable"; throw answerError; }
}
